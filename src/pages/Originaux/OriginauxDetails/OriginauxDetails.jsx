...
const OriginauxDetails = () => {
  ...
  const originauxFiltered = originauxData.filter((item) => {
    return item.dimension === size;
  });

  // Trouve l'œuvre sélectionnée par son id
  const selectedImage = originauxFiltered.find(
    (item) => item.id === Number(id)
  );

  const currentIndex = originauxFiltered.findIndex(
    (item) => item.id === Number(id)
  );
  ...
  const nextIndex = (currentIndex + 1) % originauxFiltered.length;
  const nextItem = originauxFiltered[nextIndex];
